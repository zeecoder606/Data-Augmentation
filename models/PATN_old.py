...
class TransferModel(BaseModel):
    ...
    def forward(self):
        ...
        self.fake_p2,self.mask_1,self.fg_img,self.bg_img,self.bg_fill, self.mask_2,self.out_patn_img,self.target_img_bg,self.target_img_fg = self.netG(G_input)
        # bg_fill_target_ = self.bg_fill*mask_2
        # bg_target_gt = self.input_P2*mask_2
    ...
    def backward_G(self):
        ...
        bg_in = self.bg_fill*(1.0-self.mask_1)
        bg_gt = self.input_P2*(1.0-self.mask_2)
        # fg_gt = self.input_P2*(self.mask_2)

        loss_in_bg = self.L1_loss(self.bg_img, bg_in)
        loss_bg = self.L1_loss(self.target_img_bg, bg_gt)
        # loss_fg = self.L1_loss(self.target_img_fg, fg_gt)
        ...
        self.loss_G_L1 = self.L1_loss(self.fake_p2, self.input_P2) * self.opt.lambda_A
        ...
        if self.opt.with_D_PB or self.opt.with_D_PP:
            pair_loss = pair_L1loss + pair_GANloss + loss_in_bg*0.5 + loss_bg*0.5
        else:
    ...
    #     #background consistency loss
        ...
        # bg_gt = self.input_P2*(1.0-self.mask_2)
        # # fg_gt = self.input_P2*(self.mask_2)
        # # bg_target_gt = self.input_P2*(1-self.mask_2)
        # # loss_bg = self.target_img_bg
    ...
    #     if self.opt.with_D_PB or self.opt.with_D_PP:
    #         # pair_loss = loss_in_bg*5 + loss_bg*5 + loss_fg*5 + pair_GANloss
    #         pair_loss = loss_in_bg + loss_bg+ pair_L1loss + pair_GANloss 
    #     else:
    #         pair_loss = pair_L1loss
    ...
    def get_current_visuals(self):
        ...
        fg_img_ = util.tensor2im(self.fg_img.data)
        bg_img_ = util.tensor2im(self.bg_img.data)
        ...
        mask_1 = (self.mask_1[0,:,:,:]).detach().cpu().numpy()
        # print(mask_1.shape, "....................")
        mask_1 = ((np.transpose(mask_1,(1,2,0)))*255).astype(np.uint8())

        mask_2 = (self.mask_2[0, :, :, :]).detach().cpu().numpy()
        # print(mask_2.shape, "....................")
        mask_2 = ((np.transpose(mask_2,(1,2,0)))*255).astype(np.uint8())

        vis = np.zeros((height, width*13, 3)).astype(np.uint8) #h, w, c
        vis[:, :width, :] = input_P1
        vis[:, width:width*2, :] = input_BP1
        vis[:, width*2:width*3, :] = input_P2
        vis[:, width*3:width*4, :] = input_BP2
        vis[:, width*4:width*5, :] = fake_p2
        vis[:, width*5:width*6, :] = mask_1
        vis[:, width*6:width*7, :] = fg_img_
        vis[:, width * 7:width * 8, :] = bg_img_
        vis[:, width * 8:width * 9, :] = bg_img_fill_
        vis[:, width * 9:width * 10, :] = out_patn_img_
        vis[:, width * 10:width * 11, :] = mask_2
        vis[:, width * 11:width * 12, :] = target_img_bg_
        vis[:, width * 12:, :] = target_img_fg_
