...
# from cmd_1 import args
import generate_pose 
import create_pairs_dataset
import argparse
...
def compute_cordinates(heatmap_avg, paf_avg, th1=0.1, th2=0.05):
    ...
    for part in range(18):
        ...
        list_peak = list(peaks)
        length = len(list_peak)
        peaks_with_score = [x + (map_ori[x[1],x[0]],) for x in (list_peak)]
        
        id = range(peak_counter, peak_counter + length)
...
if phase == "train":
    input_path = '../seed_data/train'
    annotations_file = '../seed_data/seed-annotation-train.csv'
    save_pose_path = '../seed_data/trainK'
    pairs_file_train = '../seed_data/seed-pairs-train.csv'
elif phase == "test_in":
    input_path = '../seed_data/test_in/'
    annotations_file = '../seed_data/seed-annotation-test_in.csv'
    save_pose_path = '../seed_data/test_inK'
else:
    input_path = '../seed_data/test_out'
    annotations_file = '../seed_data/seed-annotation-test_out.csv'
    save_pose_path = '../seed_data/test_outK'
...
if os.path.exists(annotations_file):
    processed_names = set(pd.read_csv(annotations_file, sep=':')['name'])
    result_file = open(annotations_file, 'a')
else:
    result_file = open(annotations_file, 'w')
    processed_names = set()
...
for image_name in tqdm(img_list):
    ...
    oriImg = imread(os.path.join(input_path, image_name))[:, :, ::-1]  # B,G,R order
